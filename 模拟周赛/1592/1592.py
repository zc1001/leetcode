...
class Solution(object):
    def reorderSpaces(self, text):
        ...
        if(len(str_new) == 0):
            return ''
        if(len(str_new) == 1):
            str_middle += str_new[0]
            for i in range(bl_count):
                str_middle += ' '
            return str_middle

        bl_right_num = 0
        if(bl_count % ( len(str_new) - 1)  == 0):
            bl_num = bl_count / ( len(str_new) - 1)
        else:
            bl_num = math.floor( bl_count / ( len(str_new) - 1) )
            bl_right_num = bl_count - bl_num *(len(str_new) - 1)


        for i in range(len(str_new)-1):
            str_middle += str_new[i]
            for j in range(int(bl_num)):
                str_middle += ' '
        str_middle += str_new[len(str_new) - 1]
        if(bl_right_num >0):
            for i in range(bl_right_num):
                str_middle += ' '
        return str_middle
...
if __name__ == '__main__':
    solution  = Solution()
    s = solution.reorderSpaces(" practice   makes   perfect")
    print(s)
